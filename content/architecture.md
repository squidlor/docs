...
Squidlor is one system with three faces: contracts on chain, a set of backend services off chain, and the interfaces people actually touch. Since August 2026 the same oracle also settles a prediction market and quotes a launchpad, both on Base.
...
```text
  ...
  (CEXs, Yahoo,      │ pyth-pusher · sports-pusher              │  push event outcomes
   ...
   ON CHAIN  │ SquidlorAdapterV2 → SquidPriceFeed (per asset)       │
             ...
             │ Base only: MarketFactory · FPMM · ConditionalTokens  │
...
| **ponder** | Indexes the market contracts on Base (creations, trades, resolutions) into the database `quest` reads. Judge its health by indexed block against chain head, not by process status. |
...
| **oracle-chat** | [Oracle Chat](/ai/oracle-chat): seven desks over live feed state, wallets, Virtuals agent tokens, prediction markets and stock-paired launches, live at [chat.squidlor.com](https://chat.squidlor.com). Its server also hosts the launchpad routes the trade page uses. |
...
| **trade** | [squidlor.trade](https://squidlor.trade): the stock-paired token board, token pages, buy and sell, and the native launch form. See [stock-paired tokens](/products/trade). |
...
The practical consequence: bringing Squidlor to a new EVM chain is a deploy plus a configuration change. Neither the Robinhood Chain launch nor the Base launch required a Solidity change.
